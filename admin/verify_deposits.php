...
foreach ($deposits as $d) {
    ...
    elseif ($d['status'] === 'completed') $completed_count++;
...
<div class="mb-8 admin-fade">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold" style="color:var(--color-text-primary)">Verify Deposits</h1>
            <p class="mt-1 text-sm" style="color:var(--color-text-muted)">Review and approve company deposit requests.</p>
        </div>
        <div class="flex items-center gap-2 text-sm" style="color:var(--color-text-muted)">
            ...
            <span>Total: <strong style="color:var(--color-text-primary)"><?= count($deposits) ?></strong> deposits</span>
        </div>
    </div>
</div>
    ...
    <div class="admin-stat-card card admin-fade delay-2">
        <div class="flex items-start justify-between">
            <div>
                ...
                <p class="text-3xl font-extrabold mt-1 text-emerald-600"><?= $completed_count ?></p>
            </div>
...
<div class="card admin-fade delay-5" style="border-radius:var(--co-radius,0.875rem);overflow:hidden;">
    <?php if (empty($deposits)): ?>
        <div class="py-16 text-center">
            ...
            <p class="text-lg font-semibold" style="color:var(--color-text-secondary)">No deposit requests</p>
            <p class="text-sm mt-1" style="color:var(--color-text-muted)">When companies request a deposit, they will appear here.</p>
        </div>
    <?php else: ?>
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr style="background:var(--color-border-light,#f8fafc);border-bottom:1px solid var(--color-border)">
                        <th class="py-3.5 px-5 font-semibold text-xs uppercase tracking-wider" style="color:var(--color-text-muted)">Deposit</th>
                        <th class="py-3.5 px-5 font-semibold text-xs uppercase tracking-wider" style="color:var(--color-text-muted)">Company</th>
                        <th class="py-3.5 px-5 font-semibold text-xs uppercase tracking-wider" style="color:var(--color-text-muted)">Amount</th>
                        <th class="py-3.5 px-5 font-semibold text-xs uppercase tracking-wider" style="color:var(--color-text-muted)">Method & Ref</th>
                        <th class="py-3.5 px-5 font-semibold text-xs uppercase tracking-wider" style="color:var(--color-text-muted)">Proof</th>
                ...
                <tbody>
                    <?php foreach ($deposits as $d): ?>
                        <tr class="group" style="border-bottom:1px solid var(--color-border);transition:background 0.15s">
                            <td class="py-4 px-5">
                                <div class="text-xs font-mono mb-0.5" style="color:var(--color-text-placeholder)">#<?= $d['id'] ?></div>
                                <div class="font-medium" style="color:var(--color-text-primary)"><?= date('M j, Y', strtotime($d['created_at'])) ?></div>
                                <div class="text-xs" style="color:var(--color-text-muted)"><?= date('g:i A', strtotime($d['created_at'])) ?></div>
                            </td>
                            <td class="py-4 px-5">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                                        <?= e(mb_strtoupper(mb_substr($d['company_name'] ?: $d['username'], 0, 1))) ?>
                                    </div>
                                    <div class="min-w-0">
                                        <div class="font-semibold truncate" style="color:var(--color-text-primary)"><?= e($d['company_name'] ?: $d['username']) ?></div>
                                        <div class="text-xs" style="color:var(--color-text-muted)">@<?= e($d['username']) ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="py-4 px-5">
                                <span class="text-lg font-bold text-emerald-600 dark:text-emerald-400">+<?= number_format($d['amount'], 2) ?> MMK</span>
                            </td>
                            <td class="py-4 px-5">
                                ...
                                <?php if (!empty($d['transaction_id'])): ?>
                                    <div class="text-xs mt-1 truncate max-w-[180px]" style="color:var(--color-text-muted)" title="<?= e($d['transaction_id']) ?>">
                            ...
                            <td class="py-4 px-5">
                                <?php if (!empty($d['proof_image'])): ?>
                                    <a href="<?= e(base_url($d['proof_image'])) ?>" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all" style="background:rgba(99,102,241,0.1);color:#4f46e5" onmouseover="this.style.background='rgba(99,102,241,0.18)'" onmouseout="this.style.background='rgba(99,102,241,0.1)'">
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                        View Proof
                                    </a>
                                <?php else: ?>
                                    <span class="text-xs italic" style="color:var(--color-text-placeholder)">No proof</span>
                                <?php endif; ?>
                            </td>
                            <td class="py-4 px-5">
                            ...
                            <td class="py-4 px-5 text-right">
                                <?php if ($d['status'] === 'pending'): ?>
                                    <div class="flex items-center justify-end gap-2">
                                        <form method="POST" action="verify_deposits.php" class="inline-block" onsubmit="return confirm('Are you sure you want to APPROVE this deposit?');">
                                        ...
                                        <button type="button" onclick="openRejectModal(<?= $d['id'] ?>)" class="action-btn action-btn-reject" title="Reject">
...
<div id="rejectModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4" style="background:rgba(0,0,0,0.6);backdrop-filter:blur(4px)">
    <div class="relative w-full max-w-md rounded-2xl shadow-2xl overflow-hidden" style="background:var(--color-card);border:1px solid var(--color-border)">
        <div class="px-6 py-4 flex justify-between items-center" style="border-bottom:1px solid var(--color-border);background:var(--color-border-light,#f8fafc)">
            <h3 class="text-lg font-bold" style="color:var(--color-text-primary)">Reject Deposit</h3>
        ...
        <form method="POST" action="verify_deposits.php" class="p-6">
            ...
            <div class="mb-5">
                <label class="block text-sm font-semibold mb-1.5" style="color:var(--color-text-secondary)">Rejection Reason <span class="text-red-500">*</span></label>
                <textarea name="rejection_reason" required rows="3" class="form-input w-full" placeholder="e.g., Payment screenshot is invalid or missing..."></textarea>
            </div>
